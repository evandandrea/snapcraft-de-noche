...
docker run --privileged -v "$(pwd)":/snapcraft -v "$TRAVIS_BUILD_DIR":/build -v /tmp:/tmp ubuntu:xenial sh -c "apt update && apt install locales -y && locale-gen en_US.UTF-8 && export LC_ALL=en_US.UTF-8 && cd /snapcraft && apt build-dep -y ./ && /build/snap.sh $repo $repo_branch $ppa"
